	...
	public function widget( $args, $instance ) {
		...
		( "
			...
			FROM $wpdb->posts as posts
				...
				INNER JOIN $wpdb->term_taxonomy as tt3 ON tr3.term_taxonomy_id = tt3.term_taxonomy_id" . $from ."
	...
	function form(
        ...
        if ( $custom_taxonomies ) {
            foreach ( $custom_taxonomies  as $custom_taxonomy ) {
                ...
                ?>
                <p>
                    <label for="<?php echo $this->get_field_id( $custom_taxonomy->name.'_id' ); ?>"><?php echo( $custom_taxonomy->label.':' ); ?></label><br/>
                    <select id="<?php echo $this->get_field_id( $custom_taxonomy->name.'_id' ); ?>"
                            name="<?php echo $this->get_field_name( $custom_taxonomy->name.'_id' ); ?>[]" size=3 multiple="multiple"
                            class="widefat"
                            style="height: auto;">
                        <?php
